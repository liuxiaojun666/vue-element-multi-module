...
function getParams(key) {
    let item = process.argv.slice(2).find(item => item.split('=')[0] === key)
...
const devWebpackConfig = merge(baseWebpackConfig, {
    ...
    plugins: [
        new webpack.DefinePlugin({
            ...
            // 'process.defineEnv': {
            //     'process.proxyTarget': JSON.stringify(filterArgs('-proxyTarget') || process.env.API_DEV)
            // }
        }),
...
module.exports = new Promise((resolve, reject) => {
    ...
    portfinder.getPort((err, port) => {
        if (err) {
            ...
            let host = ['localhost', '127.0.0.1', '0.0.0.0', 'local.solway.cn'].includes(devWebpackConfig.devServer.host)
                ? 'local.solway.cn'
            ...
            devWebpackConfig.plugins.push(new FriendlyErrorsPlugin({
                compilationSuccessInfo: {
                    messages: [
                        ...
                        - ProxyTarget:   {bold.cyan ${multiConfig.process.proxyTable['/api'].target}}
